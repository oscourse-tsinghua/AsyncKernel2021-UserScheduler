...
线程仅作为CPU的抽象，与协程灵活绑定，每个CPU有且仅有一个进程，封装CPU的执行环境，内核栈地址，内核页表，用户栈等，每个换上CPU执行的任务都使用这个线程的用户栈，当任务被强制中断时，用户栈被该任务拿走，（谁？）为其分配一个新的用户栈。
...
1. 一个CPU：内核无任务，用户进程占据CPU，中断到来，陷入内核，保存用户进程上下文于内核，处理中断，返回，修改pcb中的context。

2. 多核：
   
   1. 内核没有在CPU上运行，与单核相同。
   
   2. 内核在其他CPU上运行，1.可以类似于单核处理；2.将中断转给内核CPU处理，协程占据的这个CPU将等待中断返回，此时将下一个任务换上CPU。
...
每个正在运行的执行单位（传统线程或协程）都有自己的栈，由于支持函数的调用/返回机制。

栈帧（stack frame）：调用一个函数，会生成一个栈帧压入栈顶，栈帧包括**返回地址、参数、局部变量**。函数返回时，栈帧出栈。

只需要分配出栈空间，将栈底地址写入`sp`寄存器，编译器就会自动将函数的参数、局部变量与返回地址生成栈帧压入栈中。

所以，此处的主要工作是思考栈的分配策略与封装方式。
