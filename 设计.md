...
```rust
pub struct Context {
    pub registers: [usize;寄存器数], // cpu 所有寄存器的值
    pub satp: usize, // 一级页表起始地址，rv中保存于satp寄存器
    ...
}
```
...
内核进程占据CPU，用户进程响应，外部中断。

一个CPU：内核无任务，用户进程占据中断到来，陷入内核，保存用户进程上下文于内核，处理中断，返回，修改pcb中的context

多核：内核没有在CPU上运行，与单核相同。内核在其他CPU上运行，1.可以类似于单核处理；2.将中断转给内核CPU处理，自己这个CPU切换下一个任务

协程的实际执行内容为Future，这意味着协程只能在特定位置让出（返回Pending），要使其能即时响应中断，必须保存上下文，**由此引出问题1：在内核栈保存上下文，那么在哪保存内核栈？**
...
在传统的线程调度器中，线程通过函数栈执行线程的主（子）函数或系统调用（内核栈）；在协程调度器中，协程的执行不需要栈来保存执行内容（函数），执行内容就是future本身，因此不需要shi用户栈，但是，陷入内核后执行内核任务（中断服务例程），需要把内核任务Pin在一个内存地址中，类似与使用内核栈执行系统调用。 **问题3：如何把任务固定在该空间？**
